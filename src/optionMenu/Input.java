...
public class Input {
	...
	public boolean confirmInput(String input) {
		String confirm;
		while (true) {
			System.out.println(String.format("Please enter Y if comfortable with input:%s, N if not.", input));
			confirm = scanner.nextLine().toLowerCase();
			if (confirm.equals("n")) {
				logger.info(String.format("Not confirming input, input: %s", input));
				return false;
			} else if (confirm.equals("y")) {
				logger.info(String.format("Input confirmed, input:%s", input));
				return true;
			}
		}
	}
	...
	public int inputInteger(String prompt, boolean confirm) {
		...
		while (!isIntegerInputted) {
			...
			if (scanner.hasNextInt()) {
				...
				if (!confirm) {
					logger.info(String.format("Integer inputted, returned: %s.Prompt: %s ", choice, prompt));
					return choice;
				}
	...
	public String inputOriginalString(String prompt, boolean confirm) {
		...
		logger.info(String.format("String inputted, returned: %s.Prompt: %s", stringInput, prompt));
		return stringInput;
	}
		...
		while (!isDateInputted) {
			...
			if (isValidDate(dateInput)) {
				try {
					...
					System.out.println("Invalid date format. Please try again. Expected format (YYYY-MM-DD)");
				}
			} else {
				System.out.println("Invalid date format. Please try again. Expected format (YYYY-MM-DD)");
			}
		}
		logger.info(String.format("Date inputted, returned: %s.Prompt: %s", date, prompt));
		return dateInput;
	}
